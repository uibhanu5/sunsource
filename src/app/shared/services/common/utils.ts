...
export default class Utils {
  ...
  static dateWithDateMonthYear(inputDateString: string) {
    const inputDate = new Date(inputDateString);
    const date = String(inputDate.getDate() < 10 ? '0' + inputDate.getDate() : inputDate.getDate());
    const month = String((inputDate.getMonth() + 1) < 10 ? '0' + (inputDate.getMonth() + 1) : (inputDate.getMonth() + 1));
    const fullyear = String(inputDate.getFullYear());
  ...
  static getTime(inputString: string): any {
    try {
  ...
  static getTimeFromString(inputString: string):any {
    try {
      const arr = inputString.split(' ');
  ...
  static stringify(input: string[]): any {
    try {
      if (environment.logger.enabled === true) {
  ...
  static convertStringToDate(dateStr: any) {
    const parts = dateStr.split('/');
    return new Date(parts[2], parts[1] - 1, parts[0]);
  }

  static getISOStringFromServerDateString(dateStr: any) {
  ...
  static getDateFromServerDateString(dateStr: any) {
    if (Utils.isValidInput(dateStr)) {
      const dateonly = dateStr.split(' ');
      const parts = dateonly[0].split('-');
 ...
 static getDateBySeperator(dateStr: string | number | Date, seperator: string) {
  ...
  static convertBooleanToString(input: any) {
    if (Utils.isValidInput(input)) {
  ...
  static convertBooleanToNumber(input: any) {
    if (Utils.isValidInput(input)) {
      if (input === true || input === 'true') {
        return 0;
      } else {
