...
export class BaseClass implements OnInit {
   ...
   */
  isValidField(groupName: any, fieldName: string) {
  ...
  getErrorMessage(formGroup: any, validation_messages: any, validation_item: any): string {
    let message = '';
  ...
  findInvalidControls(formObject: FormGroup) {
    const invalid = [];
   ...
   * @param event Keyboard event
   */
  allowNumberAndDot(event: KeyboardEvent) {
